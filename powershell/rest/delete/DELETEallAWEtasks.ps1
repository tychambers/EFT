<#
Deletes all AWE tasks on a site
...
#deletes all AWE tasks
foreach($awe in $aweList.data){
$awe_id = $awe.id
$aweTask = Invoke-RestMethod -Uri "$baseURL/v2/sites/$siteID/awe-tasks/$awe_id" -Method 'DELETE' -Headers $authHeader}