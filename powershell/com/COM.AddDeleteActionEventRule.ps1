...
foreach($erType in $eventRuleTypes){
    ...
    For ($iRule = 0; $iRule -lt $eRules.Count(); ++$iRule){
        ...
        if ($params.Name -eq $eventRuleName)
            ...
            # Creates the upload action parameter interface and add parameters
            {$uploadActionParams = New-Object -ComObject 'SFTPComInterface.CIUploadActionParams'
            $uploadActionParams.RemotePath = "C:/newPath"
            $uploadActionParams.LocalPath = "C:/test"
            $uploadActionParams.ConnectionProfileName = "test"

            # add the action into the rule, 0 signifies the first position in the rule, could be 1, 2, etc
            $objEvent.AddActionStatement(0, $uploadActionParams)

            # deletes the original statement, position 0 has now moved to 1
            $objEvent.DeleteStatement(1)}
}}}
